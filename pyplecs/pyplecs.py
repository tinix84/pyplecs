...
def generate_variant_plecs_file(scr_filename: str, dst_filename: str, modelvars: dict):
    ...
    state = 'copy'
    for i, line in enumerate(fp_src):
...
def generate_variant_plecs_mdl(src_mdl, variant_name: str, variant_vars: dict):
    ...
    variant_mdl.filename = str(src_path_obj.parent / variant_name / src_path_obj.name).replace('.plecs', f'{variant_name}.plecs')
    generate_variant_plecs_file(scr_filename= src_mdl.filename, dst_filename= variant_mdl.filename, modelvars = variant_vars)
...
class PlecsApp():
    ...
    def open_plecs(self):
        try:
            subprocess.Popen([command], creationflags=psutil.ABOVE_NORMAL_PRIORITY_CLASS)
        except Exception:
    ...
    def check_simulation_running(self, plecs_mdl):
...
class PlecsServer:
    def __init__(self, sim_path=None, sim_name=None, port='1080', load=True):
        ...
        if ((not sim_path is None) or (not sim_name is None)) and load:
            self.server.plecs.load(self.sim_path + '//' + self.sim_name)
        else:
            print('sim_path or sim_path is invalid')
    ...
    def close(self):
        ...
        self.server.plecs.close(self.sim_path + '//' + self.sim_name)

    def load_model_vars(self, data):
        varin = {**self.optStruct['ModelVars'], **data}
        for k, v in varin.items():
            varin[k] = float(varin[k]) # why????
        optStruct = {'ModelVars': varin}
        return opts
...
class GenericConverterPlecsMdl:
    def __init__(self, filename: str):
        ...
        # simulation vars
        self.optStruct = self.default_model_vars()
        # I/O section
    ...
    def write_modelvars_struct_to_plecs(self):
        # TODO: read from workspace write to plecs file
        pass

    def default_model_vars(self):
        varin = dict()
        varin['Vi'] = 0.0
        varin['Vo'] = 0.0
