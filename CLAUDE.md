...
- **Cache identity is a composite key** (ADR-0010). `pyplecs/cache/topology.py` canonicalizes the parsed `.plecs` block tree into a Topology Document; `pyplecs/cache/identity.py` derives the four ids. The key never claims equality it cannot prove — extend the keep/drop tables there, never loosen the degrade path.
...
| 2026-08-28 | Cache Record identity is a composite topology key ([ADR-0010](docs/adr/0010-cache-record-identity-is-a-composite-topology-key.md)) | The byte hash busted on every cosmetic edit and ignored the PLECS version. Four ids — topology, params, solver, environment — each conservative: unrecognised constructs degrade to bytes per construct, unknown environment disables caching. Supersedes the charting premise that the Circuit Model is the canonicalizer's input. |
| 2026-08-28 | Simulation MCP Server is a second server ([ADR-0011](docs/adr/0011-two-mcp-servers-two-trust-levels.md)) | A docs server is safe to hand anyone; a server that starts PLECS is not. `pyplecs-mcp-sim` carries the Simulation Task lifecycle over stdio submit/poll with disjoint tool names; the eight docs tools stay frozen. |
| 2026-08-28 | Design Quantities on demand, never cached ([ADR-0012](docs/adr/0012-design-quantities-are-computed-on-demand.md)) | Stress, efficiency and losses are a pure function of a Simulation Result plus a caller-declared Signal Map; storing them would need an analysis version in the Cache Key. Roles are declared, never inferred from names. |
| 2026-08-28 | Live verification is opt-in and metric-based ([ADR-0013](docs/adr/0013-live-verification-is-opt-in-and-metric-based.md)) | PLECS is licensed and host-bound, so live checks are selected by marker (`live_plecs`, `converter_acceptance`), probe availability, run under isolated config and never gate a push. Waveform samples are not stable identities: pass/fail rests on steady-state metrics against a recorded reference plus analytic invariants; every run leaves an Evidence Bundle. |
