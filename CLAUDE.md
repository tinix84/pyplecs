...
| 2026-08-28 | Live verification is opt-in and compares Design Quantities, never samples ([ADR-0013](docs/adr/0013-live-verification-is-opt-in-and-compares-design-quantities.md)) | PLECS is licensed and host-bound, so live checks are selected by marker (`live_plecs`, `converter_acceptance`), probe availability, run under isolated config and never gate a push. Waveform samples are not stable identities: pass/fail rests on steady-state Design Quantities against a recorded reference plus analytic invariants; every run leaves an Evidence Bundle. |
